...
class WeekEditorFreeplayState extends MusicBeatState
{
	...
	function addEditorBox() {
		...
		var storyModeButton:FlxButton = new FlxButton(0, 685, "Story Mode", function() {
			MusicBeatState.switchState(new StageEditorState(weekFile));
		...
		var saveWeekButton:FlxButton = new FlxButton(0, 685, "Save Week", function() {
			StageEditorState.saveWeek(weekFile);
		});
		saveWeekButton.screenCenter(X);
		saveWeekButton.x += 120;
		add(saveWeekButton);
	...
	override function update(elapsed:Float) {
		if(StageEditorState.loadedWeek != null) {
			super.update(elapsed);
			FlxTransitionableState.skipNextTransIn = true;
			FlxTransitionableState.skipNextTransOut = true;
			MusicBeatState.switchState(new WeekEditorFreeplayState(StageEditorState.loadedWeek));
			StageEditorState.loadedWeek = null;
			return;
		}
